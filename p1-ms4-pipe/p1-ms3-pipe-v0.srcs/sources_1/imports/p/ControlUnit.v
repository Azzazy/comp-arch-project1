...
module ControlUnit (
    ...
    always @(*) begin
        ...
        if((opcode== `OPCODE_SYSTEM)||(opcode==`OPCODE_FENCE))begin
            ...
            Branch   =   (opcode[4:2] ==  3'b110)? 1:0;//branch, jal and jalr instructions
            MemRead  =   (opcode ==  `OPCODE_Load)? 1:0;
            ...
            case (opcode)  
                ...
                default:          ALUOp = 1; 
            endcase
            MemWrite =   (opcode[4:2] ==  2);//store instructions
            ALUSrc   =   ((opcode    ==  `OPCODE_Load)|| (opcode == `OPCODE_Store) || (opcode == `OPCODE_Arith_I) || (opcode    ==  `OPCODE_JALR) || (opcode == `OPCODE_LUI));   
            RegWrite =   ((opcode    ==  `OPCODE_Load) || (opcode ==`OPCODE_Arith_I) || (opcode==`OPCODE_JALR) || (opcode == `OPCODE_JAL) ||(opcode ==  `OPCODE_AUIPC)|| (opcode == `OPCODE_LUI)||(opcode == `OPCODE_Arith_R));
