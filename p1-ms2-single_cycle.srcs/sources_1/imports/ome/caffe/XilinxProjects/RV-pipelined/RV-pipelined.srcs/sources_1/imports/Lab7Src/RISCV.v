...
module RISCV (
    ...
    wire [31:0] PC_out, PCAdder_out, BranchAdder_out, PC_in, 
        ...
        ALU_out, Mem_out, Inst, regWSrcMuxOut;
    ...
    assign PCSrc[0] = Zero && Branch;
    assign PCSrc[1] = (Inst[6:2]==5'b11001)?1'b1:1'b0;//for jalr
    ...
    ControlUnit cu(.opcode(Inst[6:2]),.Branch(Branch),.MemRead(MemRead),.MemToReg(MemToReg)
        ,.ALUOp(ALUOp),.MemWrite(MemWrite),.ALUSrc(ALUSrc),.RegWrite(RegWrite),.shamtSrc(shamtSrc));
