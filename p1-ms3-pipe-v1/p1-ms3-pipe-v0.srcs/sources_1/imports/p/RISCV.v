...
//PIPELINE REGISTER WIRES
   ...
   EX_rs2_mux_out, single_mem_out, IF_ID_Inst_raw;
   wire [13:0]  ID_EX_Ctrl, ID_EX_Ctrl_MUX;
 ...
 //PIPELINE REGISTER WIRES END
    ...
    reg clk2;
    wire add4pc;
    wire [13:0]ID_EX_in_Ctrl;
    assign add4pc = (IF_ID_Inst_raw[0] || Inst_raw[0]) && (IF_ID_Inst_raw[1] || Inst_raw[1]);//selection to increment pc by 4 or 2
    assign ID_EX_in_Ctrl = {branch_jalr, shamtSrc, ALUSrc, ALUOp, MemRead, MemWrite, by, half, unsign,MemToReg, RegWmux2Ctl, RegWrite};
    ...
    //PIPELINE REGISTERS
    RegWLoad #(128) IF_ID (clk,rst,(1'b1 && (~stall)),
                            {PC_out,Inst, PCAdder_out, Inst_raw},
                            {IF_ID_PC,IF_ID_Inst, IF_ID_PCAdder_out, IF_ID_Inst_raw});

    RegWLoad #(193)  ID_EX   (clk, rst, (1'b1 && (~stall)),
                                {IF_ID_Inst[30], IF_ID_Inst[`IR_funct3],ID_EX_Ctrl_MUX, rs1_src ,IF_ID_Inst[`IR_rs2] , IF_ID_Inst[`IR_rd], ImmGen_out, RegR1, RegR2,
    ...
    RegWLoad pc(clk,rst,(~stall) && (~clk2) && (~halt),PC_in,PC_out);//change the updating of the PC
    ...
    Mux2_1 #(14) ID_EX_stall(stall, ID_EX_in_Ctrl, 0, ID_EX_Ctrl_MUX);
    //EX STAGE END
    //MEM STAGE
    ...
    always @(*) begin
        case(ledSel)
            ...
            2: leds <= {MEM_WB_Ctrl[0],l_zero,Branch_con, EX_MEM_Ctrl[8], MemToReg, ALUOp, MemWrite, 
                        ALUSrc, RegWrite, Zero, stall, ID_B, ID_A};
            3: leds <=Inst_raw[15:0];
            default: leds <= 0;            
        endcase
